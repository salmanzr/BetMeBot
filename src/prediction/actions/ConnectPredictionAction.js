...
export function updateDynamicPredictionData(address) {
  ...
  return async function(dispatch, getState) {
    ...
    dispatch(updateVoteBalances(address));
    dispatch(updateVotersBalances(address));
...
export function updateVotersBalances(address) {
  return async function(dispatch, getState) {
    const prediction = getState().prediction;
    const contract = getState().prediction.contract;
    const web3 = getState().network.web3;
    const voter = getState().network.activeAccountAddress;
    console.log('get voter balances');
    prediction.voterPositiveBalance = +web3.fromWei(await contract.getVoterBalance(true, {from: voter}), 'ether').toNumber();
    if (!util.checkContinue(address, getState)) return;
    prediction.voterNegativeBalance = +web3.fromWei(await contract.getVoterBalance(false, {from: voter}), 'ether').toNumber();
    if (!util.checkContinue(address, getState)) return;
    util.cachePrediction(address, prediction);
    dispatch({
      type: UPDATE_PREDICTION,
...
export function updateVoteBalances(address) {
  return async function(dispatch, getState) {
    const prediction = getState().prediction;
    const contract = getState().prediction.contract;
    const web3 = getState().network.web3;
    ...
    const preview = getState().market.previews[address];
    if(preview) preview.balance = prediction.balance;
    prediction.positiveVotesBalance = +web3.fromWei(await contract.voteTotals.call(true), 'ether').toNumber();
    if (!util.checkContinue(address, getState)) return;
    prediction.negativeVotesBalance = +web3.fromWei(await contract.voteTotals.call(false), 'ether').toNumber();
    if (!util.checkContinue(address, getState)) return;
    util.cachePrediction(address, prediction);
    dispatch({
      type: UPDATE_PREDICTION,
