...
import {DEBUG_MODE, TARGET_LIVE_NETWORK} from '../../constants';
import {
  createPrediction
} from '../actions';
...
class CreatePredictionComponent extends React.Component {
  ...
  componentWillMount() {
    ...
    let betEndDate = TARGET_LIVE_NETWORK === 'rinkeby' ? dateUtil.unixToDate(now + 60) : dateUtil.unixToDate( now + dateUtil.daysToSeconds( 7 ) );
    this.props.form.onValues({
      statement: DEBUG_MODE ? miscUtil.makeid() : '',
      betEndDate,
      withdrawPeriodDays: DEBUG_MODE || TARGET_LIVE_NETWORK === 'rinkeby' ? 0 : 7,
      withdrawPeriodHours: 0,
      withdrawPeriodMinutes: DEBUG_MODE || TARGET_LIVE_NETWORK === 'rinkeby' ? 1 : 0,
    });
  }
